...
## 2026-09-13 — Public portfolio release preparation

- Replaced machine-specific setup notes with portable local and private-tunnel guidance.
- Reworked the README around product capabilities, architecture, verified limitations and safe provider configuration.
- Added a current dark-workspace screenshot, security policy, GitHub Actions CI and a tested static-frontend Pages workflow.
- Security baseline: no tracked credentials or common secret patterns found; dependency audit reports zero vulnerabilities.
- Verification: 27 Node.js tests, 25 Playwright tests, production build and `git diff --check` passed before publication.
- The public demo is frontend-only. The unauthenticated single-user backend remains intentionally private.
...
- **Real verification:** two short live streaming document questions through the browser → app → a private compatible gateway, one with each configured model, returned HTTP 200 and cited the correct synthetic document lines. These were real inference calls; monetary cost was not calculated. No personal documents were used.
...
- No dependency additions, Mini AlgoChat changes, shared-provider changes or public backend exposure were made in this stage. `.env` and generated artifacts stayed ignored.
...
- Local ignored `.env` enabled two provider model identifiers. Both were checked against the private gateway's model listing. Listing is not proof of successful generation or a cost guarantee; no paid inference was made in this stage.
...
- Backend provider configuration, dependencies, Mini AlgoChat and shared services were untouched. No paid requests or deployment occurred. Test servers closed automatically.

## 2026-09-11 — Separate project foundation

- Created a separate `advance-online-chatbot` workspace at the user's request.
...
- Preserved the working chat foundation and configured a Git-ignored private provider connection with no copied secrets.
- Started a separate Git repository without inherited deployment configuration. Removed inherited deployment scripts and retained dependency versions for a minimal fork.
...
- Test servers stopped automatically and generated artifacts remained ignored.
