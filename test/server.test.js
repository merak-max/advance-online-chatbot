...
test("model list is controlled, deduplicated, and includes the default", async (t) => {
  const url = await listen(t, createServer(createApp({ OPENAI_MODEL: "default", OPENAI_ALLOWED_MODELS: "other, other, , default" })));
  assert.deepEqual(await (await fetch(`${url}/api/models`)).json(), { models: ["default", "other"], defaultModel: "default" });
  for (const model of [null, {}, "", "unapproved"]) {
    assert.equal((await post(url, { messages: [message], model })).status, 400);
  }
});

test("selected model reaches the SDK provider and is returned for attribution", async (t) => {
  let received;
  const provider = await listen(t, createServer(async (req, res) => {
    let body = "";
    for await (const chunk of req) body += chunk;
    received = JSON.parse(body).model;
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ object: "response", output: [{ type: "message", content: [{ type: "output_text", text: "Model test" }] }] }));
  }));
  const url = await listen(t, createServer(createApp({ OPENAI_API_KEY: "test-only", OPENAI_BASE_URL: provider, OPENAI_MODEL: "first", OPENAI_ALLOWED_MODELS: "second" })));
  const response = await post(url, { messages: [message], model: "second" });
  assert.equal(response.status, 200);
  assert.equal((await response.json()).model, "second");
  assert.equal(received, "second");
  await post(url, { messages: [message] });
